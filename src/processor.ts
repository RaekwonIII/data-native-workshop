...
import * as sink from "./abi/sink";
import { Contract, Account, ContractHistoricalBalance } from "./model";
  ...
  for (const block of ctx.blocks) {
    for (const item of block.items) {
      if (
        ...
        const event = sink.decodeEvent(item.event.args.data);
        if (event.__kind === "ValueTransferred") {
          records.push({
            id: item.event.id,
            from: ss58.codec(5).encode(event.from),
            amount: event.value,
